...
- 核心的思路在于，公共祖先的节点，其有三种可能；(1) p在left树，q在right树 (2) p或者q为公共祖先，剩余节点在其子树中
- 如果左右节点均为空，且当前节点非p和q，则返回空
- 如果当前节点为q或者p，则返回当前节点；
- 直到左右节点均不为空为止，即为公共祖先
...
- 本题的核心思路在于遍历到4以后，其左右均为空；即有2个'#'；其字符串为4##，在这种情况下，将其替换为#
- 同理将1也替换为#，这样可以实现对3进行消解；
- 以此类推，当最后的栈中只剩下一个元素时，其为正确的先序遍历结果
